...
用于新建或明显更新前端原型、后台原型、主题、项目文档等资源时，先收敛产品需求，再确认设计方案。原型浏览工具属于工作台基础设施，修改它时参考 `rules/architecture-guide.md`。对齐贯穿全过程：读取资料、生成规格、计划实施和验收时，只要发现会改变方向、范围、成本或验收标准的问题，都应回到相应阶段继续确认。
...
2. 当前目录最近的 `AGENTS.md`、`CONTEXT.md`、`README.md` 和相关 `rules/`。
...
- 这是前端原型、后台原型，还是原型浏览工具能力。
- 目标用户和核心任务。
- 本次范围、功能清单和不做什么。
- 页面或资源的核心内容、数据来源和素材来源。
- 关键状态、核心路径和必要交互。
- 用户最终如何判断结果可用。
- 真实业务逻辑与原型演示边界：哪些规则、数据读写和状态属于正式开发，哪些只是 mock 展示。
- 真实数据、流程、状态和权限文档的来源；若尚不存在，记录为待补文档或待确认项。
...
- 实施边界：本轮会做什么，不会做什么。
- 对象边界：业务页面放入 `src/prototypes/`，浏览工具能力放入 `tools/`。
- 所有页面必须确认正常、加载、空、异常等页面级状态；不适用的状态需要说明原因。
- 复杂页面还必须确认权限、角色、会员等级或业务状态变体。
- 必须确认原型中的 mock 数据、假成功、固定延时、静态弹窗等是否仅用于演示，以及对应的正式开发行为。
- 存在独立 PRD 时，以 PRD 的完整追踪矩阵作为唯一主表；SPEC 只记录当前页面涉及的需求和验收 ID 子集。
- PRD 初稿中的页面 / SPEC 可以暂记“待设计”，但进入实现前必须补齐所有 Must/P0 需求的页面、状态和验收映射。
