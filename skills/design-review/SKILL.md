...
5. 项目级审核规则：`rules/ui-review-guide.md`（如有）。其中的工具层、前端内容层、后台内容层边界，以及图标、导航、尺寸、可访问性和严重度定义，优先于本 Skill 的通用补充规则。
...
执行 SuperIM 项目审核时，必须额外检查 `rules/ui-review-guide.md` 中的：

- 工具层 / 前端内容层 / 后台内容层边界；
- `lucide-react`、`@ant-design/icons` 的分层使用与 12/16/20/24px glyph 尺寸；
- 前端 65px 二级页头、44px 内容行，以及后台 240px Sider、64px Header 的布局基线；
- 44×44px 操作区域、键盘焦点、active/loading/empty/error 等状态；
- Critical / Warning / Info 的项目交付门槛。
