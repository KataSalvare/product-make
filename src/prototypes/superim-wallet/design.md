...
| 垂直内边距 | `12px` 上下 | 对应既有 `py-3` |
| 页头高度 | `65px` | `12px` 上下内边距 + 40px 内容行 + 1px 底部边框，与编辑资料、安全设置一致 |
| 内容行高 | `40px` | 对齐基础页的 `p-2` + 24px 图标；钱包内部滚动区负责长内容滚动 |
...
- 左侧区域采用返回按钮与标题的 `12px` 间距。
- 返回按钮视觉图标为 24px；使用 `p-2 -ml-2` 的 40px 盒子，图标视觉起点和编辑资料、安全设置一致。
