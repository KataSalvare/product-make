...
聊天转账属于会话内操作，不使用钱包二级页面承载：

- 单聊点击 Transfer 后，直接在当前聊天容器内打开 `Transfer in chat` 弹窗，并默认展示当前会话对象。
- 群聊点击 Transfer 后，先打开群成员选择器；用户选定一名成员后，在同一群聊容器内打开转账弹窗。
- 抽屉使用聊天容器级遮罩，从底部进入并贴齐容器底边，宽度铺满容器（宽屏最大 420px），仅保留顶部圆角；同时提供统一页头返回/关闭按钮和 Escape 关闭，关闭后保留原聊天上下文。
- `/wallet/chat-transfer` 和 `/wallet/transfer` 只作为历史链接兼容入口，不代表新的聊天交互需要跳转到二级页面。
