    ...
    <div className={`p-6 rounded-xl bg-gradient-to-br ${colorClasses[color]} border backdrop-blur-sm`}>
      <div className="flex items-center justify-between mb-4">
        <div className="p-3 rounded-lg bg-white/10">
          {React.cloneElement(icon as React.ReactElement<{ className?: string; size?: number }>, { className: 'text-white', size: 24 })}
        </div>
        {trend !== undefined && (
...
const PieChart: React.FC<{ data: { name: string; value: number; color: string }[] }> = ({ data }) => {
  ...
  const slices = data.reduce<Array<{ item: typeof data[number]; startAngle: number; endAngle: number }>>((acc, item) => {
    const angle = (item.value / total) * 360;
    const startAngle = acc.length > 0 ? acc[acc.length - 1].endAngle : 0;
    acc.push({ item, startAngle, endAngle: startAngle + angle });
    return acc;
  }, []);
  ...
  return (
    <div className="flex items-center gap-4">
      <svg width="100" height="100" viewBox="0 0 100 100">
        {slices.map(({ item, startAngle, endAngle }, index) => {
