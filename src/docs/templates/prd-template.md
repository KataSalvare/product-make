...
> 本节描述正式产品应如何工作，不描述原型中的 mock 实现。具体接口地址、请求响应 Schema 和错误码可放在数据模型或研发接口文档中，但必须在此说明业务语义。

| 交互 ID | 触发条件 | 前置条件 / 权限 | 真实输入 | 业务处理与数据读写 | 成功结果 / 状态变化 | 失败、空数据与重试 | 副作用 / 埋点 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `INT-01` | `[用户或系统触发动作]` | `[登录态、角色、会员等级、对象状态]` | `[字段、格式、必填规则]` | `[读取/创建/更新/删除及业务规则]` | `[返回结果、页面状态、下一步]` | `[错误、空数据、超时、重试规则]` | `[通知、日志、埋点等]` |

**原型边界**：`[原型使用的 mock 数据、静态值、固定延时、假成功或简化流程]`；**正式开发行为**：`[真实数据源、持久化、状态流转和权限校验]`；**未决项**：`[尚未确定的接口或业务规则]`。
...
1. `AC-01-01` Given 用户打开新建文档弹窗<br>
   When 选择 `文档类型=RPD`<br>
   Then 模板列表仅展示 `RPD` 相关模板。

2. `AC-01-02` Given 当前类型下没有模板<br>
   When 用户查看模板区<br>
...
1. `AC-02-01` Given 用户点击某模板卡片<br>
   When 打开预览<br>
   Then 可看到章节目录、示例段落、更新时间。

2. `AC-02-02` Given 预览打开<br>
   When 用户切换模板<br>
...
1. `AC-03-01` Given 用户选择模板并点击“创建”<br>
   When 系统生成文档<br>
   Then 模板中的 `{{project_name}}`、`{{owner}}`、`{{date}}` 被替换为当前上下文值。

2. `AC-03-02` Given 模板包含未提供值的变量<br>
   When 生成文档<br>
...
1. `AC-04-01` Given 用户打开新建文档弹窗<br>
   When 有历史记录<br>
   Then 列表顶部展示最近使用模板（最多 `3` 个）。

2. `AC-04-02` Given 没有历史记录<br>
   When 查看模板区<br>
   Then 默认按“团队推荐”排序展示。
...
## 6. 需求追踪矩阵（Requirement Traceability）

> 交给研发实施的 PRD 必须填写。本表是需求、页面 SPEC 与验收标准之间的唯一完整追踪主表；页面 SPEC 只保留当前页面涉及的 ID 子集，不复制整张主表。PRD 初稿阶段允许将“页面 / SPEC”标记为“待设计”，但进入实现前必须补齐所有 Must/P0 需求的页面、状态和验收映射。

| PRD 需求 ID | 需求摘要 | 页面 / SPEC | 页面区域或状态 | 验收 ID | 备注 |
| --- | --- | --- | --- | --- | --- |
| `US-01` | 按文档类型筛选模板 | `template-create-spec.md` | 类型选择器、模板列表 | `AC-01-01`、`AC-01-02` | Must |
| `US-03` | 基于模板创建文档 | `template-create-spec.md` | 变量输入区、创建成功态 | `AC-03-01`、`AC-03-02` | Must |

**完整性检查**：不能存在没有页面承接的 Must 需求，也不能存在没有 PRD 需求或验收标准的核心页面操作。
...
- [ ] 需求追踪矩阵完整，`US-*` 或 `FR-* → SPEC 页面/区域/状态 → AC-*` 可逐项回溯。
...
5. 交给研发实施的 PRD 必须补充需求追踪矩阵，并为页面级状态和验收标准分配唯一编号。
6. 编号统一使用 `US-01` / `FR-01`、`PS-01`、`AC-01-01`，不得使用 `US/FR-01` 等组合写法。
7. 研发交付的 PRD 必须填写真实数据交互逻辑，并明确原型 mock 与正式开发行为的边界。
