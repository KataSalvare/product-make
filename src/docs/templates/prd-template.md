...
> 复杂页面或多页面功能必须填写。每条用户故事/功能需求使用唯一 `US-*` 或 `FR-*` 编号，每条验收标准使用唯一 `AC-*` 编号；页面 SPEC、页面区域和页面级状态也要能回溯到对应编号。
...
- [ ] 需求追踪矩阵完整，`US/FR → SPEC 页面/区域/状态 → AC` 可逐项回溯。
...
5. 复杂页面必须补充需求追踪矩阵，并为页面级状态和验收标准分配唯一编号。
