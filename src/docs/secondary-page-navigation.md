...
- 内边距：上下 `12px`、左右 `16px`，对应 `px-4 py-3`；宽屏只扩大内容区，不改变页头起点。
- 高度：`65px`，由 `12px + 40px 内容行 + 1px 边框` 组成。
- 返回按钮：`p-2 -ml-2` 的 40×40px 视觉盒子，使用 24px `ArrowLeft`，颜色为 `var(--on-surface)`。
